/**

 @Name：order 管理员管理模块控制
 @Author：偶木
 @Date：2018-08-28
    
 */

 
layui.define(['table', 'form'], function(exports){
	var $ = layui.$,
	admin = layui.admin,
	view = layui.view,
	table = layui.table,
	form = layui.form;
	
	//管理员列表
	table.render({
		elem: '#OM-confine-user',
		url: '/manager/confine/user/info', //模拟接口
		cols: [[
			{type: 'checkbox', fixed: 'left'},
			{field: 'id', title: '编号'},
			{field: 'name', title: '姓名',},
			{field: 'basics_money', title: '基本工资'},
			{field: 'money', title: '奖金'},
			{field: 'dname', title: '部门'},
			{field: 'title', title: '等级'},
			{title: '操作', width: 200, align: 'center', fixed: 'right', toolbar: '#table-toolbar-user'}
		]],
		page: true,
		limit: 30,
		height: 'full-320',
		
		//ajax
		contentType: 'application/json; charset=UTF-8',
		method: 'post',
		request: {
			limitName: 'size'
		},
		response: {
			statusCode: 200
		},
		text: '对不起，加载出现异常！'
	});
  
	//管理员列表_监听工具条
	table.on('tool(OM-confine-user)', function(obj){
		var data = obj.data;
		if(obj.event === 'edit'){
			view('OM-module-temp').render('power/component/manager-edit', data).done(function() {
				//执行 Ajax 后重载
          		$.ajax({
					type: "POST",
					url: "/manager/confine/role/info",
					dataType:'json',//预期服务器返回的数据类型
					contentType: "application/json; charset=utf-8",
					async: false,
					success: function(res){
						var list = res.data;
						for(var i=0; list.length > i; i++) {
							var obj = list[i];
							if(obj.id == data.role_id) {
								$("#m_role").append("<option value="+obj.id+" selected>"+obj.title+"</option>");
							} else {
								$("#m_role").append("<option value="+obj.id+">"+obj.title+"</option>");
							}
						}
					}, error: function(){
						location.hash='/system/404';
					}
	    		});
          		
				//执行 Ajax 后重载
          		$.ajax({
					type: "POST",
					url: "/manager/confine/user/employee",
					dataType:'json',//预期服务器返回的数据类型
					contentType: "application/json; charset=utf-8",
					async: false,
					success: function(res) {
						var list = res.data;
						for(var i=0; list.length > i; i++) {
							var obj = list[i];
							if(obj.id == data.id) {
								$("#m_user").append("<option value="+obj.id+" selected>"+obj.name+"</option>");
							} else {
								$("#m_user").append("<option value="+obj.id+">"+obj.name+"</option>");
							}
						}
					}, error: function(){
						location.hash='/system/404';
					}
	    		});
				
          		//动态加载
				form.render();
				
				//提交监听
				form.on('submit(OM-form-submit)', function(data){
		    		var field = data.field;

		    		//提交
		    		$.ajax({
		    			type: "POST",
						url: "/manager/confine/user/update",
						dataType:'json',//预期服务器返回的数据类型
						contentType: "application/json; charset=utf-8",
						async: true,
						data: JSON.stringify(field),
						success: function(data){
							//直接跳转不做任何处理且提示
							layer.open({
	                  	      	content: data.msg,
	                   	     	btn: ['确认'],
	                   	     	yes: function(index, layero) {
	                        		layer.close(index);
	                        		//刷新当前页面
	        			    		window.location.reload();
	                        	}
	                   	 	});
						}, error: function(){
							location.hash='/system/404';
						}
		    		});
		    		return false;
		  		});
			});
		}
	});
	
	//管理员角色
	table.render({
		elem: '#OM-confine-role',
		url: '/manager/confine/role/info',
		cols: [[
			{type: 'checkbox', fixed: 'left'},
			{field: 'id', title: '编号', width: 200},
			{field: 'title', title: '角色名称'},
			{field: 'description', title: '角色描述'},
			{title: '操作', width: 200, align: 'center', fixed: 'right', toolbar: '#table-toolbar-role'}
		]],
		page: false,
		limit: 30,
		height: 'full-320',
		
		//ajax
		contentType: 'application/json; charset=UTF-8',
		method: 'post',
		request: {
			limitName: 'size'
		},
		response: {
			statusCode: 200
		},
		text: '对不起，加载出现异常！'
	});
  
	//管理员角色_监听工具条
	table.on('tool(OM-confine-role)', function(obj){
		var data = obj.data;
		if(obj.event === 'del'){
			layer.confirm('确定删除此管理员角色？', function(index) {
				layer.close(index);
				
				//数据封装
				var req = {"id": data.id};
				//执行 Ajax 后重载
	          	$.ajax({
					type: "POST",
					url: "/manager/confine/role/delete",
					dataType:'json',//预期服务器返回的数据类型
					contentType: "application/json; charset=utf-8",
					async: false,
					data: JSON.stringify(req),
					success: function(data){
						layer.open({
	                        content: data.msg,
	                        btn: ['确认'],
	                        yes: function(index, layero) {
	                        	layer.close(index);
	                        	table.reload('OM-confine-role', null);
	                        }
	                    });
					}, error: function(){
						location.hash='/system/404';
					}
		    	});
			});
		} else if(obj.event === 'edit'){
			//执行 Ajax 后重载
      		$.ajax({
				type: "POST",
				url: "/manager/confine/role/menu",
				dataType:'json',//预期服务器返回的数据类型
				contentType: "application/json; charset=utf-8",
				async: false,
				data: JSON.stringify({}),
				success: function(res){
					var ver = data.power.split(',');
					for(var i=0; i < res.data.length; i++) {
						var list = res.data[i].list;
						for(var j=0; j < list.length; j++) {
							var obj = list[j];
							var val2 = false;
							for(var z=0; z < ver.length; z++) {
								if(obj.id == parseInt(ver[z])) {
									val2 = true;
									break;
								}
							}
							obj.ioc = val2;
						}
					}
					data.data = res.data;
				}, error: function(){
					location.hash='/system/404';
				}
    		});
      		console.log(data.data);
      		//
			view('OM-module-temp').render('power/component/role-edit', data).done(function() {
				//动态加载
				form.render();
				//提交监听
				form.on('submit(OM-form-submit)', function(data2){
					var field = data2.field;
					
					//获取checkbox[name='like']的值
		            var arr = new Array();
		            $("input:checkbox[name='m_power']:checked").each(function(i){
		                arr[i] = $(this).val();
		            });
		           	field.power = arr.join(",");//将数组合并成字符串
					
					//提交
		    		$.ajax({
		    			type: "POST",
						url: "/manager/confine/role/update",
						dataType:'json',//预期服务器返回的数据类型
						contentType: "application/json; charset=utf-8",
						async: false,
						data: JSON.stringify(field),
						success: function(data){
							//直接跳转不做任何处理且提示
							layer.open({
	                  	      	content: data.msg,
	                   	     	btn: ['确认'],
	                   	     	yes: function(index, layero) {
	                        		layer.close(index);
	                        		//刷新当前页面
	        			    		window.location.reload();
	                        	}
	                   	 	});
						}, error: function(){
							location.hash='/system/404';
						}
		    		});
		    		return false;
				});
			});
		}
	});
	
	//系统操作日志
	table.render({
		elem: '#OM-confine-log',
		url: '/manager/confine/log/info',
		cols: [[
			{type: 'checkbox', fixed: 'left'},
			{field: 'id', title: '编号'},
			{field: 'name', title: '用户名'},
			{field: 'info', title: '日志内容'},
			{field: 'ip', title: '操作IP'},
			{field: 'type', title: '操作类型'},
			{field: 'state', title: '日志状态'},
			{field: 'add_date', title: '操作时间'},
			{title: '操作', width: 150, align: 'center', fixed: 'right', toolbar: '#table-toolbar-log'}
		]],
		page: true,
		limit: 30,
		height: 'full-320',
		
		//ajax
		contentType: 'application/json; charset=UTF-8',
		method: 'post',
		request: {
			limitName: 'size'
		},
		response: {
			statusCode: 200
		},
		
		text: '对不起，加载出现异常！'
	});
  
	//系统操作日志_监听工具条
	table.on('tool(OM-confine-log)', function(obj){
		var data = obj.data;
		layer.confirm('确定删除此系统日志？', function(index){
			layer.close(index);
			
			//数据封装
			var req = {"id": data.id};
			//执行 Ajax 后重载
          	$.ajax({
				type: "POST",
				url: "/manager/confine/log/delete",
				dataType:'json',//预期服务器返回的数据类型
				contentType: "application/json; charset=utf-8",
				async: false,
				data: JSON.stringify(req),
				success: function(data){
					layer.open({
                        content: data.msg,
                        btn: ['确认'],
                        yes: function(index, layero) {
                        	layer.close(index);
                        	table.reload('OM-system-log', null);
                        	window.history.pushState(null, null, '/start/index.html#/power/log');
                        }
                    });
				}, error: function(){
					location.hash='/system/404';
				}
	    	});
		});
	});
	
	//部门信息
	table.render({
		elem: '#OM-confine-department',
		url: '/manager/confine/department/info',
		cols: [[
			{type: 'checkbox', fixed: 'left'},
			{field: 'id', title: '编号'},
			{field: 'department_id', title: '第三方部门ID'},
			{field: 'name', title: '部门名称'},
		]],
		page: true,
		limit: 30,
		height: 'full-320',
		
		//ajax
		contentType: 'application/json; charset=UTF-8',
		method: 'post',
		request: {
			limitName: 'size'
		},
		response: {
			statusCode: 200
		},
		
		text: '对不起，加载出现异常！'
	});
	
	exports('power', {})
});